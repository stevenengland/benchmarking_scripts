...
def run_benchmark_with_list_of_sql_statements(
    ...
    print(f"Will execute the queries {count} times with {wait} second(s) between runs.")
    print()
    try:
...
def run_benchmark_with_same_sql_multiple_times(
    ...
    print(f"Will execute the query {count} times with {wait} second(s) between runs.")
    print()
    try:
        for i in range(1, count + 1):
            with oracledb.connect(connection_string) as connection:
